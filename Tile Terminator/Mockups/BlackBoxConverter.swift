...
class BlackBoxConverter {
        ...
        do {
            ...
            for song in songs {
                ...
                if (files != nil) {
                    for file in files! {
                        if (file.contains(".sm")) {
                            songsList.append(b.convertStepfile(dirPath: songPath, filePath: songPath + file, songTimePad: 3.0))
                        }
                    }
                }
            }
    ...
    public func convertStepfile (dirPath: String, filePath: String, songTimePad: Double) -> Song {
        ...
        for i in 0..<difficulties.count {
            ...
            formattedDifficulties.append(Difficulty(notes: cSongNotes, difficultyName: difficultyNames[i], difficulty: difficultyValues[i]))
        }
